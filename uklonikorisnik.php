...
$sql = "SELECT * FROM vozac";
$result = mysqli_query($dbc, $sql);


if (isset($_POST['delete_vozac'])) {
    $vozac_id = $_POST['delete_vozac'];

    // Delete the selected driver from the "vozac" table
    $delete_sql = "DELETE FROM vozac WHERE id_vozac = '$vozac_id'";
    if (mysqli_query($dbc, $delete_sql)) {
        $msg = "Vozač uspješno uklonjen.";
        header("Refresh:0");
    } else {
        $msg = "Greška prilikom brisanja vozača: " . mysqli_error($connection);
    }
}
?>
...
<body>
    ...
    <div class="container">
        <div class="sredina">
            <div class="naslovi">
                <h2 style="margin-bottom: 30px;">UKLONI VOZAČA</h2>
            </div>
            <?php
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo '<p>ID vozača: ' . htmlspecialchars($row["id_vozac"]) . '</p>';
                    echo '<p>Ime: ' . htmlspecialchars($row["vozac_ime"]) . '</p>';
                    echo '<p>Prezime: ' . htmlspecialchars($row["vozac_prezime"]) . '</p>';
                    echo '<p>Adresa: ' . htmlspecialchars($row["vozac_adresa"]) . '</p>';
                    echo '<p>Datum rođenja: ' . htmlspecialchars($row["vozac_datum_rod"]) . '</p>';
                    echo '<form method="POST">';
                    echo '<input type="hidden" name="delete_vozac" value="' . $row["id_vozac"] . '">';
                    echo '<button type="submit" class="btn btn-outline-dark gumb">Ukloni</button>';
                    echo '</form>';
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
