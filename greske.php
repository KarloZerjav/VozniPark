...
$sql = "SELECT id_greske, kod_greske, opis_greske FROM greske";
    ...
    <div class="container">
        <div class="sredina">
            ...
            <form enctype="multipart/form-data" action="" method="POST">
                <table class="table table-hover" id="greske">
                    ...
                    <tbody class="font_mob">
                        <?php
                        if ($result && mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo
                                '<tr onclick="location.href=\'opisgreske.php?id_greske=' . $row["id_greske"] . '\';" style="cursor: pointer;">
                                    <td>' . $row["kod_greske"] . '</td>
                                    <td>' . $row["opis_greske"] . '</td>
                                </tr>';
                            }
                        } else {
                            echo '<tr style="text-align:center"><td colspan="2">Nema dostupnih podataka</td></tr>';
                        }
                        ?>
                    </tbody>
                ...
                <div class="sredina_povijest">
                    <a href="dodajgresku.php"><button type="button" class="btn btn-outline-dark gumb"><img style="width: 30px;" src="img/add-file.png" alt=""></button></a>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        $('#greske').DataTable({
            ...
            "paging": true, // Disable pagination for better responsiveness
