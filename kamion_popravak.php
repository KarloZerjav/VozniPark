...
$povijestRows = array();
$povijestRowsKamion = array();

if (isset($_GET['id_kamion']) && isset($_GET['id_vrste'])) {
    ...
    $queryPovijestKamion = "SELECT pk.id_vrste, pk.kilometri, pk.datum, vp.naziv_popravka, vp.opis 
                            ...
                            INNER JOIN vrste_popravka vp ON pk.id_vrste = vp.id_vrste 
                            WHERE pk.id_kamion = $id AND pk.id_vrste = $id_vrste"; // Add condition for id_vrste
...
if (isset($_POST['update'])) {
    $registrationId = $_GET['id_kamion'];
    $id_vrste = $_GET['id_vrste']; // Assuming you're getting id_vrste from the URL
    $datum = $_POST['datum_kamion'];
    $kilometri = $_POST['kilometri_kamion'];
    $naziv_popravka = $_POST['naziv_popravka']; // Assuming 'naziv_popravka' comes from the 'naslov' input field
    $opis = $_POST['opis_kamion'];

    // Update data in 'popravak_kamion' table
    $updateQuery1 = "UPDATE popravak_kamion SET datum=?, kilometri=? WHERE id_kamion=? AND id_vrste=?";
    $stmtUpdate1 = mysqli_prepare($dbc, $updateQuery1);
    mysqli_stmt_bind_param($stmtUpdate1, 'ssii', $datum, $kilometri, $registrationId, $id_vrste);
...
if (isset($_POST['delete'])) {
    ...
    $deleteQueryPopravakKamion = "DELETE FROM popravak_kamion WHERE id_vrste=? AND id_kamion=?";
    $stmtDeletePopravakKamion = mysqli_prepare($dbc, $deleteQueryPopravakKamion);
    mysqli_stmt_bind_param($stmtDeletePopravakKamion, 'ii', $id_vrste, $id_kamion);

    // Check if the statement executed successfully
    if (mysqli_stmt_execute($stmtDeletePopravakKamion)) {
        mysqli_stmt_close($stmtDeletePopravakKamion);
                ...
                <div class="col">
                    <form enctype="multipart/form-data" action="" method="POST">
                        <div class="form-group forma_servis row">
                            ...
                            <div class="col-sm-9">
                                <?php foreach ($povijestRowsKamion as $povijestRow) : ?>
                        ...
                        <div class="form-group forma_servis row">
                            ...
                            <div class="col-sm-9">
                                <?php foreach ($povijestRowsKamion as $povijestRow) : ?>
                                    <input type="date" name="datum_kamion" class="form-control forma_input" value="<?php echo $povijestRow["datum"]; ?>">
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <div class="form-group forma_servis row">
                            <label for="kilometri_kamion" class="col-sm-3">Kilometri:</label>
                            <div class="col-sm-9">
                                <?php foreach ($povijestRowsKamion as $povijestRow) : ?>
                                    <input type="text" name="kilometri_kamion" class="form-control forma_input" value="<?php echo $povijestRow["kilometri"]; ?>">
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <div class="form-group forma_servis row">
                            ...
                            <div class="col-sm-9">
                                <?php foreach ($povijestRowsKamion as $povijestRow) : ?>
                                    <textarea name="opis_kamion" class="form-control forma_input" rows="2" cols="70"><?php echo $povijestRow["opis"]; ?></textarea>
                                <?php endforeach; ?>
                            </div>
                        </div>
